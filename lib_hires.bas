...
DIM hi_mask0(8) AS BYTE @_hi_mask0
_hi_mask0:
DATA AS BYTE $7f, $bf, $df, $ef, $f7, $fb, $fd, $fe
DIM hi_mask1(8) AS BYTE @_hi_mask1
_hi_mask1:
...
TYPE TypeHires
    ...
    SUB Clear(Color0 AS BYTE, Color1 AS BYTE) STATIC
        ...
        MEMSET THIS.screen_mem_addr, 1000, SHL(Color0, 4) OR Color1
    END SUB
...
bitmap_plot_end:
    ...
    SUB Text(x AS BYTE, y AS BYTE, text AS STRING * 40, CharMemAddr AS WORD) STATIC
        ZP_W0 = SHL(CWORD(y), 3)
        ZP_W0 = SHL(y_tbl_hi(ZP_W0), 8) + y_tbl_lo(ZP_W0) + SHL(CWORD(x), 3)
    ...
    SUB Text(x AS BYTE, y AS BYTE, text AS STRING * 40) STATIC
        rem disable interrupt and enable char rom
        ASM
            sei
        END ASM
        POKE 1, %11111010        
        CALL Text(x, y, text, Color, $d800)
        POKE 1, %11111110
        ASM
            cli
        END ASM
